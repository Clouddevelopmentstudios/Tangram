...
gfx::Size BrowserViewLayout::GetMinimumSize() {
  ...
  if (bookmark_bar_ && bookmark_bar_->visible() &&
...
void BrowserViewLayout::OpenURL(std::wstring strURL,
  ...
  //bool bNewTab = (url == GURL("chrome://newtab/"));
  WindowOpenDisposition nPos = (WindowOpenDisposition)nDisposition;
 /* switch (nPos) {
    case WindowOpenDisposition::CURRENT_TAB: {
      if (bNewTab)
        nPos = WindowOpenDisposition::CURRENT_TAB;
      else
        nPos = WindowOpenDisposition::NEW_FOREGROUND_TAB;
    } break;
