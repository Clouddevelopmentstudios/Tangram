...
void RenderFrameHostImpl::ExecuteJavaScript(const base::string16& javascript) {
  ...
  Send(
      new FrameMsg_JavaScriptExecuteRequest(routing_id_, javascript, 0, false));
}
...
void RenderFrameHostImpl::ExecuteJavaScript(
  ...
  Send(new FrameMsg_JavaScriptExecuteRequest(routing_id_, javascript, key,
                                             true));
...
bool RenderFrameHostImpl::CreateRenderFrame(int proxy_routing_id,
  ...
  if (proxy_routing_id != MSG_ROUTING_NONE) {
    RenderFrameProxyHost* proxy =
        RenderFrameProxyHost::FromID(GetProcess()->GetID(), proxy_routing_id);
...
bool RenderFrameHostImpl::CanCommitOrigin(const url::Origin& origin,
                                          const GURL& url) {
